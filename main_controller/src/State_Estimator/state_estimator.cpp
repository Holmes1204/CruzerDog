...
void StateEstimate::UpdateOrientationData(state_data _state_interior)
{
...
void StateEstimate::InitParameters()
{
    double dt = 0.01;
    _xhat.setZero();
    _ps.setZero();
...
void StateEstimate::LinearKF(Eigen::Matrix<double, 3, quad::NUM_LEG> _foot_p, Eigen::Matrix<double, 3, quad::NUM_LEG> _foot_v, Eigen::Vector4d _contact_phase)
{
    ...
    /*!（2）定义三维矩阵 */
    Eigen::Vector3d g(0, 0, -9.81);                              // 1）定义重力矩阵
    Eigen::Matrix3d Rbod = this->state_data_.rBody_.transpose(); // 2）定义身体坐标系下的旋转矩阵，通过四元素转换而来
    Eigen::Vector3d a = this->state_data_.aWorld_ + g;           // 3）定义世界坐标系下的加速度矩阵

    /*!（3）定义四维矩阵 */
    Eigen::Vector4d pzs = Eigen::Vector4d::Zero();
    Eigen::Vector4d trusts = Eigen::Vector4d::Zero();
    ...
    {
        int i1 = 3 * i;
        /*            Eigen::Vector3d ph;                                        //计算相对于CoM的髋部位置，即机器人坐标系中腿的臀部位置
         */
        Eigen::Vector3d p_rel = _foot_p.block<3, 1>(0, i);                                   //计算世界坐标系下机器人真正的位置= 机器人坐标系中腿的臀部位置+腿的位置
        Eigen::Vector3d dp_rel = _foot_v.block<3, 1>(0, i);                                  //计算世界坐标系下机器人真正的速度= 状态估计器中每条的速度
        Eigen::Vector3d p_f = Rbod * p_rel;                                                  //计算身体坐标系下机器人真正的位置= 身体坐标系下的旋转矩阵*世界坐标系下机器人真正的位置
        Eigen::Vector3d dp_f = Rbod * (this->state_data_.omega_body_.cross(p_rel) + dp_rel); //计算身体坐标系下机器人真正的速度=     身体坐标系下的旋转矩阵*（躯干坐标系中的角速度正交阵+世界坐标系下机器人真正的速度）
 ...
 * */
Eigen::Matrix<double, 3, 3> StateEstimate::getData()
{
